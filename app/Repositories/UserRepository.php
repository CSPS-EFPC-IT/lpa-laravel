...
class UserRepository extends BaseEloquentRepository
{
    ...
    protected $ldapUsernameAttribute;
    ...
    {
        parent::__construct();
        $this->ldapBaseDN = config('ldap.connections.default.settings.base_dn');
        $this->ldapUsernameAttribute = config('ldap_auth.identifiers.ldap.locate_users_by');
    }
    ...
    public function searchLdap($search, $limit = self::LDAP_SEARCH_LIMIT)
    {
        return Adldap::search()
            ->setDn($this->ldapBaseDN)
            ->whereHas($this->ldapUsernameAttribute)
            ->whereHas('mail')
            ->whereHas('givenname')
            ->whereHas('sn')
    ...
    {
        if ($user = Adldap::search()->where([$this->ldapUsernameAttribute => $username])->first()) {
            return $user;
        }
