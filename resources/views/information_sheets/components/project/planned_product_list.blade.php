@form(['form' => $data])
    ...
    @form_section(['title' => trans_choice('forms/planned_product_list.tabs.planned_product', 2)])
        <h4>{{ trans_choice('forms/planned_product_list.form_section_groups.planned_product.label', 2) }}</h4>
        @forelse($formData['planned_products'] as $product)
            @field_group(['field' => 'planned_product_list.form_section_groups.planned_product.label', 'index' => $loop->iteration])
                <dl>
                    @field_learning_product_type(['data' => $product]) @endfield_learning_product_type
