...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      innerValue() {
        // here we handle the case where the select-wrap is used as-is with a multiple attribute
        // and where it is used in another wrapper, passing in multiple as a prop
        if (!_.isArray(this.value) && ((this.$attrs.multiple || this.$attrs.multiple === '') || this.$props.multiple)) {
          this.$log.error(`Attribute 'multiple' was provided but v-model is not an array for ${this.name}.`);
        }
        if (_.isArray(this.value) && (_.isUndefined(this.$attrs.multiple) && _.isUndefined(this.$props.multiple))) {
