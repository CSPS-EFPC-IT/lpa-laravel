    ...
    >
      <template v-slot:controls v-if="hasRole('owner') || hasRole('admin')">
        <el-control-wrap
          ...
          :disabled="!canEdit"
          :tooltip="$t('components.tooltip.edit_project')"
          size="mini"
        ...
        <el-control-wrap
          componentName="el-button"
          :disabled="!canDelete"
          :tooltip="$t('components.tooltip.delete_project')"
          type="danger"
          size="mini"
      ...
      <dl>
        ...
        -->
        <dd>{{ project.state ? project.state.name : $t('entities.general.none') }}</dd>
      </dl>
      <dl>
        <dt>{{ $t('entities.process.current') }}</dt>
        <dd>{{ project.current_process ? project.current_process.definition.name : $t('entities.general.none') }}</dd>
      </dl>
      <dl>
      ...
      <dl>
        <dt>{{ $t('entities.organizational_unit.director') }}</dt>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onDeleteConfirm() {
        this.confirmDelete({
          title: this.$t('components.notice.title.delete_project'),
          message: this.$t('components.notice.message.delete_project')
        }).then(() => {
          this.$emit('delete');
        }).catch(() => false);
      }
