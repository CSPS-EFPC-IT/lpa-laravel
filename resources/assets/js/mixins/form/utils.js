...
export default {
  ...
  watch: {
    '$i18n.locale'(to, from) {
      this.closeErrorNotif();
    },

    'verrors.items.length'(val) {
  ...
  methods: {
    ...
    async showRefreshErrorNotif() {
      ...
      errorNotif = await this.notifyError({
        message: this.$t('components.notice.message.validation_failure', { num: this.verrors.items.length })
      });
    },
